...
func Rollback(app IFoundation.IApplication) IConsole.Command {
	return &rollback{
...
func (this *rollback) Handle() interface{} {
	...
	var (
		...
		migrations = Collections.MustNew(this.migrations).Pluck("name")
	)
	...
	raw.Where("batch", raw.Max("batch")).Map(func(item Support.Fields) {
