...
func (client *Client) doSubscribe(topic string, fn EventCallbackFunc, serverAddr, serverPath string, subscribeType SubscribeType) {
	defer func() {
		if r := recover(); r != nil {
...
func (client *Client) Subscribe(topic string, fn EventCallbackFunc, serverAddr, serverPath string) {
...
func (client *Client) SubscribeOnce(topic string, fn EventCallbackFunc, serverAddr, serverPath string) {
...
func (service *ClientService) PushEvent(arg *ClientArg, reply *bool) error {
	service.client.eventBus.Publish(&Event{Name: arg.Topic, Data: arg.Args})
	*reply = true
	return nil
}
